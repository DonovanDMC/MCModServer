...
if (!await exists(`${tmpdir()}/upload/projectexpansion`)) {
    await mkdir(`${tmpdir()}/upload/projectexpansion`, { recursive: true });
}
...
for (const file of vDir) {
    if (!file.isFile() || !/\.jsonc?$/.test(file.name)) {
        continue;
    }
...
const publishSchema = Joi.object({
    ...
    expectedLatestCommit: Joi.string().required().regex(/^[\da-f]{40}$/),
...
const app = Router()
    .post("/publish/:id", fileHandler.single("file"), async(req: Request<{ id: string; }, unknown, {
        ...
        if (disabledVersions[req.params.id]) {
        ...
        if (gitlog) {
            if (validate(gitlog)) {
                ...
                if (!await exists(new URL(gitlogID, dataDir))) {
                    ...
                    gitlog = (await readFile(new URL(gitlogID, dataDir))).toString();
                }
            } else {
                await writeFile(new URL(gitlogID, dataDir), gitlog);
            }
        }
        if (changelog) {
            if (validate(changelog)) {
                ...
                if (!await exists(new URL(changelogID, dataDir))) {
                    ...
                    changelog = (await readFile(new URL(changelogID, dataDir))).toString();
                }
            } else {
                await writeFile(new URL(changelogID, dataDir), changelog);
            }
        }
        ...
        if (expectedLatestCommit !== latestCommit) {
            return res.status(400).json({
                success: false,
        ...
        if (!refs.some(ref => ref.includes(tag))) {
            return res.status(400).json({
                success: false,
        ...
        if (gitlogID) {
            await unlink(new URL(gitlogID, dataDir));
        }
        if (changelogID) {
            await unlink(new URL(changelogID, dataDir));
        }
