...
export default async function discord(webhookURL: string, botToken: string, mcVersion: string, version: string, changelog: string, curseforgeID: number) {
	...
	if (req.status !== 200) throw new Error(`Unexpected ${req.status} ${req.statusText} "${await req.text()}" (discord)`);
	const { id, channel_id } = (await req.json() as { id: string; channel_id: string; });
